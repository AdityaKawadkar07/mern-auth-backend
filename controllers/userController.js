...
//Trigger2FA for unkown UserAgent
    const ua = parser(req.headers['user-agent']);
...
const sendLoginCode=asyncHandler(async(req,res)=>{
    ...
    const decryptedLoginCode=cryptr.decrypt(loginCode) //Decrypt the Encrypted login code

    //Send Login Code
    const subject="Login Acces Code -AUTH:Z"
    const send_to=email;
    const sent_from=process.env.EMAIL_USER
    const reply_to='noreply@google.com'
...
const loginWithCode=asyncHandler(async(req,res)=>{
    ...
    const decryptedLoginCode=cryptr.decrypt(userToken.lToken) //Decrypt the Encrypted login code
...
const getUsers=asyncHandler(async(req,res)=>{
    const users= await User.find().sort("-createAt").select("-password")
    if(!users){
        res.status(500);
